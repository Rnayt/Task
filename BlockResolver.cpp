...
/*
GetBlockById:
1) Получаем, если есть, блок из кэша (чтение) (read lock) -> char *
2) Если блок ЕСТЬ
     - регистрируем соединение в кэше (counter++) (случай, если уже нет в кэше)
     - работает TCP хэндлер
3) Если блока НЕТ:
     - читаем с блочного устройства
     - добавляем в кэш и регистрируем
     - работает TCP хэндлер

WriteFinishedForBlock:
1) Дерегистрируем (counter--)
2) Если больше нет любителей почитать - удаляем блок из кэша
*/
